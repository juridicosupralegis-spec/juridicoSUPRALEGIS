document.addEventListener('DOMContentLoaded', () => {
    ...
    // Adaptar enlaces de WhatsApp para computadoras y celulares
    const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
    if (!isMobile) {
        document.querySelectorAll('a[href*="wa.me"]').forEach(link => {
            const href = link.getAttribute('href');
            if (href && href.includes('wa.me/')) {
                const urlParts = href.split('wa.me/');
                if (urlParts.length > 1) {
                    const phoneAndText = urlParts[1];
                    const phone = phoneAndText.split('?')[0];
                    const params = phoneAndText.includes('?') ? phoneAndText.substring(phoneAndText.indexOf('?')) : '';
                    const cleanParams = params.replace('?', '&');
                    link.setAttribute('href', `https://web.whatsapp.com/send?phone=${phone}${cleanParams}`);
                }
            }
        });
    }
    ...
    if (btnCalcular) {
        btnCalcular.addEventListener('click', () => {
            ...
            // 1. Días de vacaciones (LFT 2023 - Vacaciones Dignas)
            ...
            // 2. Aguinaldo (Proporcional a los días laborados en el año en curso)
            ...
            // Si el trabajador ingresó en el año en curso, el cálculo inicia desde su fecha de ingreso
            const startOfCalculation = (fechaIngreso > startOfYear) ? fechaIngreso : startOfYear;
            const daysInCurrentYear = Math.ceil((fechaSalida - startOfCalculation) / (1000 * 60 * 60 * 24)) || 1;
            const aguinaldo = (dailySalary * aguinaldoDiasTotal) * (daysInCurrentYear / 365);

            // 3. Vacaciones y Prima Vacacional Proporcional del año en curso
            ...
            // 4. Salario Diario Integrado (SDI) - Base para la Indemnización Constitucional
            // Integra el salario base más la parte proporcional de aguinaldo y prima vacacional
            const dailyAguinaldoFactor = (dailySalary * aguinaldoDiasTotal) / 365;
            const dailyVacationBonusFactor = (dailySalary * vacationDaysTotal * 0.25) / 365;
            const sdi = dailySalary + dailyAguinaldoFactor + dailyVacationBonusFactor;

            // 5. Indemnización Constitucional (90 días de SDI) - Solo aplica por despido injustificado
            let indemnizacion = 0;
            if (motivo === 'despido') {
                indemnizacion = sdi * 90;
            }

            // 6. Prima de Antigüedad (12 días por año, topado a 2 salarios mínimos de la zona general en 2024: $248.93 x 2)
            // LFT Art. 162: Aplica siempre en despidos; en renuncias voluntarias solo si el trabajador cumplió al menos 15 años de servicio.
            let primaAntiguedad = 0;
            if (motivo === 'despido' || (motivo === 'renuncia' && diffYears >= 15)) {
                const minWage = 248.93; // Salario mínimo 2024
                const capSalary = Math.min(dailySalary, minWage * 2);
                primaAntiguedad = capSalary * 12 * diffYears;
            }
    ...
    if (visitorStats) {
        let currentVisits = 1420; // Default base value
        try {
            // We simulate a real count starting from a professional base number
            let baseCount = 1420;
            currentVisits = parseInt(localStorage.getItem('sl_visit_count') || baseCount);
            ...
            console.warn("Storage access denied under local file:// protocol. Using default counter values.", e);
        }
